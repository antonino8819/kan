...
class KANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        Antonino: the original KAN modal, can't consider the 'dead end' nodes, i.e. tho ones
        with no connections backward. In such a situation, even if the the node value
        (x or the inner sum) is 0 (due to no backward connections), if the splines of its 
        forward connections have non zero coefficients, the node output value y is different 
        from 0. Thus, if x is 0, y is forced to 0.
        '''
        zero_mask = (x == 0)
        y[zero_mask.unsqueeze(2).expand_as(y)] = 0

        postspline = y.clone().permute(0,2,1)

        # Antonino: no scale and pruning needed (GA optimization is performed)
