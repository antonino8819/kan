...
class MultKAN(nn.Module):
    ...
    def forward(self, x, singularity_avoiding=False, y_th=10.):
        ...
        for l in range(self.depth):
            ...
            if self.save_act:
                ...
                # CIPAR #
                # make it possible to process one input at time
                self.subnode_actscale.append(MultKAN.std_safe(x, dim=0).detach())
                #########
            ...
            if self.save_act:
                ...
                # CIPAR #
                # make it possible to process one input at time
                input_range = MultKAN.std_safe(preacts, dim=0) + 0.1
                output_range_spline = MultKAN.std_safe(postacts_numerical, dim=0)  # for training, only penalize the spline part
                output_range = MultKAN.std_safe(postacts, dim=0)  # for visualization, include the contribution from both spline + symbolic
                #########
                # save edge_scale
    ...
    # make it possible to process one input at time
    @staticmethod
    def std_safe(tensor, dim=0):
        if tensor.numel() > 1:
            return torch.std(tensor, dim=dim, unbiased=False)
        else:
